...
export function VehiclesPage({ refreshKey }: { refreshKey: number; onRefresh: () => void }) {
  ...
  const handleSave = async () => {
    if (!form.customer_id || !form.make || !form.license_plate) { toast('Kötelező mezők hiányoznak', 'error'); return }
    setSaving(true)
    const payload = {
  ...
  const fuelLabels: Record<string, string> = { petrol: 'Benzin', diesel: 'Dízel', electric: 'Elektromos', hybrid: 'Hibrid', lpg: 'LPG' }
    ...
    <div className="animate-fade-in">
      <div className="flex gap-2.5 mb-4">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Rendszám, márka, VIN, tulajdonos..."
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-[#F4F5F7] border-b border-[rgba(11,30,61,0.10)]">
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#5a6a80] uppercase tracking-wider">Rendszám</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#5a6a80] uppercase tracking-wider">Jármű</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#5a6a80] uppercase tracking-wider hidden md:table-cell">Tulajdonos</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#5a6a80] uppercase tracking-wider hidden lg:table-cell">Üzemanyag</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#5a6a80] uppercase tracking-wider hidden lg:table-cell">Km-állás</th>
                ...
                <tr key={v.id} className="border-b border-[rgba(11,30,61,0.06)] hover:bg-[#fafbfc] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      ...
                      <button onClick={async () => { if (confirm('Biztosan törlöd a járművet?')) { await supabase.from('vehicles').delete().eq('id', v.id); load() } }} className="p-1.5 text-[#5a6a80] hover:text-[#C9384C]"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editVehicle ? 'Jármű szerkesztése' : 'Új jármű'}
