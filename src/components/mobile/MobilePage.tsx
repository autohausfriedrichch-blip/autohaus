...
export function MobilePage({ refreshKey }: { refreshKey: number; onRefresh: () => void }) {
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-4 p-4 bg-[#0B1E3D] rounded-[14px] text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {jobs.map(job => (
              <Card key={job.id}>
                ...
                {job.mobile_address && (
                  <div className="flex items-start gap-1.5 text-[12px] text-[#5a6a80] mb-2">
                    <MapPin size={13} className="mt-0.5 shrink-0 text-[#C9A84C]" />
                    {job.mobile_address}
                  </div>
                )}
                <div className="flex items-center gap-1.5 text-[12px] text-[#5a6a80] mb-2">
                  <Phone size={13} className="shrink-0 text-[#C9A84C]" />
                  {job.customer?.phone}
                </div>
                {job.scheduled_date && (
                  <div className="text-[11px] text-[#8fa0b5] mt-2 pt-2 border-t border-[rgba(11,30,61,0.08)]">
                    Datum: {formatDate(job.scheduled_date)} {job.scheduled_time?.slice(0,5)}
                  </div>
                )}
                {job.service_type && (
                  <div className="text-[11px] text-[#C9A84C] font-medium mt-1">{job.service_type}</div>
                )}
              </Card>
            ))}
          </div>
