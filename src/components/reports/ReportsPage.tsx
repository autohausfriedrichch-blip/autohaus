    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          ...
          {loading ? <div className="h-48 flex items-center justify-center text-[#5a6a80] text-sm">Wird geladen...</div> : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={monthlyRevenue} margin={{ top: 5, right: 5, bottom: 5, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(11,30,61,0.08)" />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: '#5a6a80' }} />
                <YAxis tick={{ fontSize: 11, fill: '#5a6a80' }} tickFormatter={v => `CHF ${(v/1000).toFixed(0)}k`} />
                <Tooltip formatter={(v: any) => [formatCurrency(v), 'Umsatz']} labelStyle={{ color: '#0B1E3D', fontWeight: 600 }} />
                <Bar dataKey="umsatz" fill="#C9A84C" radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </Card>
          ...
          <div className="space-y-4 mt-2">
            <div>
              <div className="flex justify-between text-[12px] mb-1">
                <span className="text-[#5a6a80]">Genehmigungsrate</span>
              ...
              <div className="h-2 bg-[#ECEEF2] rounded-full overflow-hidden">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-[#C9A84C] to-[#e8c96b] transition-all"
            ...
            <div className="border-t border-[rgba(11,30,61,0.08)] pt-4">
              <div className="text-[11px] font-semibold text-[#5a6a80] uppercase tracking-wider mb-3">Auftragsstatus Übersicht</div>
              {[
                ...
                { label: 'Angebote gesendet', count: stats.quotesSent || 0, color: 'bg-[#C9A84C]' },
                { label: 'Angebote genehmigt', count: stats.quotesApproved || 0, color: 'bg-emerald-400' },
              ].map(item => (
                <div key={item.label} className="flex items-center justify-between py-2 border-b border-[rgba(11,30,61,0.05)] last:border-0">
                  <div className="flex items-center gap-2">
                    <span className={`w-2.5 h-2.5 rounded-full ${item.color}`} />
                    <span className="text-[13px]">{item.label}</span>
                  </div>
                  <span className="font-semibold text-[#0B1E3D]">{item.count}</span>
                </div>
              ))}
            </div>
          </div>
